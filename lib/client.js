	...
	factory: (require) => {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Input, {
							className: ModelCapabilityCard_module_css_default.capability ?? "",
		...
		*/
		...
		function apply(ctx) {
			...
			ctx.inject(["settingsScope", "remote.session"], (scopedCtx) => {
				const scope = scopedCtx.settingsScope.bind({ namespace: SETTINGS_NS });
				const loadModels = async () => {
					return ((await scopedCtx.remote.session.modelCatalog()).groups ?? []).flatMap((g) => (g.models ?? []).map((m) => ({
						key: `${g.id}/${m.id}`,
						provider: g.id,
						model: m.id,
						name: m.name
